...
public class GridManager : MonoBehaviour
{
    ...
    // Public entry point used by external code to set the map texture and (re)generate the grid.
    public void SetMapAndGenerate(Texture2D newMap)
    {
        if (newMap == null)
        {
            Debug.LogWarning("GridManager.SetMapAndGenerate called with null map.");
            return;
        }
    ...
    private void GenerateGraph(Texture2D mapImage)
    {
        ...
        for (int z = 0; z < _height; z++)
        {
            for (int x = 0; x < _width; x++)
            {
                ...
                if (_currentTile != null && _currentTile._isNode)
                {
    ...
    private void FillGraphNeighbors(Dictionary<Node, List<Node>> _graph, Texture2D _mapImage)
    {
        foreach (KeyValuePair<Node, List<Node>> g in _graph)
        {
            ...
            if (node_z < _height - 1)
            {
                ...
                if (_upNeighborTile != null && _upNeighborTile._isNode)
                {
            ...
            if (node_z > 0)
            {
                ...
                if (_downNeighborTile != null && _downNeighborTile._isNode)
                {
            ...
            if (node_x > 0)
            {
                ...
                if (_leftNeighborTile != null && _leftNeighborTile._isNode)
                {
            ...
            if (node_x < _width - 1)
            {
                ...
                if (_rightNeighborTile != null && _rightNeighborTile._isNode)
                {
